...
function smooth2(x,y)
    ...
    return Dict("fit"=>fit,"model"=>model, "x"=>collect(0.25:0.05:2.0))
end
function smooth(x,y)
    ...
    try
        ...
        xx = np.concatenate((np.arange(0.25,3.0, 0.005), np.array([2.0])))
...
work_dir="C:\\Users\\s1.chakravar\\Documents\\Projects\\cathode_packing_data\\bimodal_distribution_database"
cd(work_dir)
nstat = collect(1:5)
nmc_wt_ratio = collect(0.75:0.05:0.92)
# nmc_wt_ratio = [0.85, 0.9]
lambda_min = [0.25, 0.5, 0.75, 1.0, 1.1, 1.2, 1.25, 1.4, 1.5, 1.6666667, 1.8, 1.9, 2.0]
...
int_utiln = si.griddata(points, util, int_points, method="nearest")
int_util = si.griddata(points, int_utiln, int_points, method="linear")
...
df_smooth= by(df_interp,[:wt_ratio,:bimodal_mix_ratio, :bimodal_radius_ratio],
           (:particle_size_ratio, :packing_density, :utilization)
               => x->(util2 = smooth2(x.particle_size_ratio, x.utilization)))
...
for brr in keys(gd_brr)
    ...
    if brr.bimodal_radius_ratio in q_brr
        ...
        if !isnothing(fit)
            ...
            plot!(xdata, ydata .* 100.0, seriestype=:scatter, labels="", color = colors[j])
