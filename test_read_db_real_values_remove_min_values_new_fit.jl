...
cd(work_dir)
cd("julia")
png(p,"test_contour_real_psi_0.285_phi_0.25.png")
cd("..")
#End Plotting
## Linear Plots
pyplot()
...
for (i,bmr) in enumerate(q_bmr)
    ...
    for brr in keys(gd_brr)
        if brr.bimodal_radius_ratio in q_brr
            ...
            df_new[!,x] = dcamsmall./l_crit_smooth
            println("--------------------")
        end
    end
...
cd("julia")
png(p,"test_real_phi_psi_0.285.png")
cd("..")
##
...
png(p, "test_real_util_psi_0.285.png")
cd("../")
##pyplot()
...
q_bmr = [0.2, 0.22, 0.25, 0.26, 0.28, 0.29, 0.3, 0.4]
# q_bmr =[ 0.25]
colors = [:red, :green, :blue, :orange, :purple, :black, :yellow, :pink]
j = 1
for (bmr, bmr2) in zip(keys(gd_bmr), keys(gd_bmr2))
    global j
    if bmr.bimodal_mix_ratio in q_bmr
        df_bmr = gd_bmr[bmr]
        df_bmr2 = gd_bmr2[bmr2]
        newx = df_bmr[!,:particle_size_ratio]
        newy = df_bmr[!,:utilization]
        newy_smooth = sf.gaussian_filter1d(newy, sigma=20.0)
        fit = get(df_bmr2[1,4], "fit", nothing)
        model = get(df_bmr2[1,4], "model", nothing)
        newx2 = get(df_bmr2[1,4], "x", nothing)
        newy2 = model(newx2, coef(fit))
        labstr = string(bmr.bimodal_mix_ratio)
        plot!(newx, newy_smooth*100.0, color= colors[j], st=:scatter, label=labstr)
...
cd(work_dir)
cd("julia")
png(p,"util_variation_psi_0.285_w_0.85_interp.png")
cd("..")
##
