...
currents = [0.2, 0.4, 0.8, 1.2] # ma/cm^2
...
for (i,d) in enumerate(dirs)
    ...
    # if ( i == 1)
        # plot!(twinx(), plated_Li, -df2[!,:ext_pressure]*1e6 .+ 0.2,
        #         color=:black, guidefontsize=16, tickfontsize = 14,
        #         label="", ylabel=L"$\sigma (MPa)$", ylim = (0.75, 3.2))
    # end
    # println(last(df2,10))
end
# annotate!()
...
cd(workdir)
png(p,"test_over_potential_plating_time.png")
