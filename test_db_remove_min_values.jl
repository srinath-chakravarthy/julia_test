...
intnan = isnan.(int_util)
int2 = int_util
for (i, int) in enumerate(intnan)
    if int
        int_util[i] = int_utilnear[i]
        int_pack[i] = int_packnear[i]
    end
end
...
p = plot(xlabel = ["" xlabel],
        ...
        grid = false)
dfx = @where(df_smooth, :wt_ratio .== 0.8)
...
colors = [:blue, :orange, :green, :black, :red, :purple]
# q_brr = [0.21,0.25, 0.275, 0.285, 0.3, 0.4]
q_brr = [0.2, 0.25, 0.285, 0.3, 0.4, 0.5]
j = 1
@. model(x,p) = 1.0/(1.0 + exp(-p[1]*(x-p[2])))
newxx = collect(0.25:0.01:findmax(lambda_min)[1])
p0 = [1.0, 0.25]
